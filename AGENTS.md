...
| File | Lines | Purpose |
|------|-------|---------|
| `leanring_buddyApp.swift` | ~140 | Menu bar app entry point. Uses `@NSApplicationDelegateAdaptor` with `CompanionAppDelegate` which creates `MenuBarPanelManager`, `PaceMenuBarOverlayManager`, and starts `CompanionManager`. No main window — the app lives entirely in menu-bar/notch surfaces. |
| `CompanionManager.swift` | ~1857 | Central state machine. Owns dictation, shortcut monitoring, screen capture, the active `BuddyPlannerClient`, the active `BuddyTTSClient`, the `LocalVLMClient`, the `PaceActionExecutor`, the `PaceVisionOCRClient`, the screen-context pre-warm task, the per-screen analysis cache, action approval, intent routing, watch-mode state, and overlay management. Tracks voice state (idle/listening/processing/responding), conversation history, and cursor visibility. Coordinates the full push-to-talk → optional text-only/watch-mode fast path → screenshot → (optional local VLM + OCR) → local planner → streaming TTS → optional approval → optional action execution → pointing pipeline. **Still oversized** — the agent loop body (~250 lines) and the screen-context service (~300 lines) are the next two splits. |
...
| `MenuBarPanelManager.swift` | ~215 | Custom NSPanel lifecycle for the floating companion panel. Manages show/hide/position, anchors to the menu-bar overlay frame, and installs click-outside-to-dismiss monitor. Does not create a visible status item. |
...
| `CompanionPanelView.swift` | ~691 | SwiftUI panel content for the menu bar dropdown. Shows companion status, push-to-talk instructions, local model status, read-screen toggle, cursor annotation toggle, action approval toggle, watch-mode toggle, permissions UI, and quit button. Dark aesthetic using `DS` design system. |
...
| `PaceScreenWatchMode.swift` | ~142 | Explicit watch-mode primitive. Samples all screens at an interval, compares fingerprints, throttles repeated changes, and emits meaningful-change events for panel/voice watch mode. |
...
| `PaceToolRegistry.swift` | ~325 | Typed local tool catalog. Defines canonical names, aliases, schema examples, descriptions, risk levels, execution summaries, observation summaries, and planner prompt generation. Future MCP-backed tools should bridge through this registry. |
| `PaceActionExecutor.swift` | ~1758 | Local action execution layer with screenshot-pixel → CG-global coordinate conversion (mirrors the pointing logic in CompanionManager). Single-clicks try `PaceAXTargeter` first; falls back to CGEvent. Also defines `PaceActionTagParser`, grouped `PaceActionExecutionPlan`, approval summaries, and tool-observation types. Parses `<tool_calls>` JSON plus legacy tags for clicks, typing, keys, scroll, app/URL opening, Music, volume, brightness, Calendar reads, Reminder creation, Finder, Notes, Mail drafts, Things, Shortcuts, and Messages opening. Action execution is gated by Info.plist `EnableActions`. |

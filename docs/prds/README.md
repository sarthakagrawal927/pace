...
| PRD | Status | Purpose |
|---|---|---|
| `unified-memory.md` | done (Phases 1–5) | One index + write pass + recall + Memory UI. Recall is UNIFIED-ONLY: semantic (LM Studio embeddings) or BM25 keyword over one index that's a superset of turns + facts + every connector source; the parallel lexical recall path is retired. `PaceLocalRetrieval` is intentionally retained as the structured operational/ingestion layer (enablement, statuses, journals, morning-brief reads, chat-history backing); full deletion was scoped and decided against (zero user-facing gain). No bundled-model dependency. |
...
| `pace-planner-v10-parameterized-actions.md` | partial (model gate) | Typed v10 envelope parsing, registry validation, schema-reject defence, Mail.draft streaming, and generic dictate/edit/setValue field streaming via `PaceStreamingPlannerFieldDetector` are wired. Grammar-constrained model-output eval gate + runtime default model switch remain queued on TinyGPT/eval-planners. |
| `pace-executor-surface.md` | partial (manual smokes) | Dispatcher surface, dry-run unit suite, and `scripts/smoke-executor-surface.sh` (dry-run + runtime hooks) are wired. Real-app AX/performance smokes (Mail latency, Safari/Notes/Slack/IDE) remain a user-run checklist. |
| `click-executor-improvements.md` | shipped (v0.3.14) | Midpoint targeting, top-K scorer, verify/retry, all-fail observations, file-driven ambiguity fixtures (`evals/click-ambiguity-fixtures/` + `PaceClickAmbiguityFixtureTests`), and runtime smoke hooks for click-target clarification + all-fail breadcrumbs. Live-app click smokes remain manual. |
